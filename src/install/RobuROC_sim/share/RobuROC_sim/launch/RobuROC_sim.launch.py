...
def generate_launch_description():
    ...
    LIDAR = IncludeLaunchDescription(velodyne_pointcloud
            PythonLaunchDescriptionSource([os.path.join(
    ...
    Pointcloud = IncludeLaunchDescription(
            PythonLaunchDescriptionSource([os.path.join(
                get_package_share_directory(PointcloudPackage),'launch','dvelodyne_transform_node-VLP16-launch.py'
            )])
    )    

    VelDriver = IncludeLaunchDescription(
            PythonLaunchDescriptionSource([os.path.join(
                get_package_share_directory(PointcloudPackage),'launch','velodyne_driver_node-VLP16-launch.py'
            )])
    )    
